...
# Ticker-Liste v1.6 inklusive Kupfer (HG=F)
TICKERS = {
    "DAX (^GDAXI)": "^GDAXI",
...
def get_market_data():
    ...
    for name, sym in TICKERS.items():
        try:
            ...
            is_fx = "EURUSD" in sym
            results.append({
                "name": name, "p": f"{p:.4f}" if is_fx else f"{p:.2f}",
                "chg": f"{chg:+.2f}%", "c_val": chg, 
                "rv": f"{rv:.2f}" if (cv > 0 and not is_fx) else "N/A",
                "al": "danger" if (rv > 3.0 and not is_fx) else "success"
            })
        except: continue
    
    # Ratio Logik (Gold/Silber)
    try:
...
<title>Radar v1.6</title><meta name="mobile-web-app-capable" content="yes">
...
<style>
    ...
    .p-val{color:#ffffff !important; font-weight:bold; font-size:1.1rem;} /* Erzwungenes Weiß */
    .lbl{color:#888;font-size:0.75rem;}
    .disc{font-size:0.6rem;color:#444;padding:15px;border-top:1px solid #222;margin-top:15px;}
</style></head>
<body><div class="container py-2"><h2 class="text-center neon mb-2">Gschmäckle Radar v1.6 🚀</h2>
...
<div class="d-flex justify-content-between align-items-center"><h6 class="mb-0" style="color:#39ff14;">{{a.name}}</h6><span class="{% if a.c_val >= 0 %}up{% else %}down{% endif %} fw-bold">{{a.chg}}</span></div>
<hr style="border-color:#333;margin:8px 0;"><div class="d-flex justify-content-between align-items-center">
<span class="lbl">PREIS</span><span class="p-val">{{a.p}}</span></div><div class="d-flex justify-content-between align-items-center">
