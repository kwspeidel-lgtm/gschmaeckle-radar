...
# VOLUMEN-QUELLE (Für den sauberen Pace/VQ)
VOL_MAPPING = {
    "Gold": "GLD", "Silber": "SLV", "Öl WTI": "USO", "Kupfer": "CPER"
}
...
def get_single_ticker_data(args):
    ...
    try:
        # 1. Preis-Daten (Future/Spot)
        t_price = yf.Ticker(symbol)
        fi = t_price.fast_info
        curr = fi['last_price']
        prev = fi['previous_close']
        if pd.isna(curr) or curr == 0:
            curr = t_price.history(period="1d")['Close'].iloc[-1]
        change = ((curr - prev) / prev) * 100 if prev else 0.0
        
        # 2. Volumen-Daten (Hybrid-Trick)
        vq_pace = None
        vol_symbol = VOL_MAPPING.get(name)
        if vol_symbol:
            t_vol = yf.Ticker(vol_symbol)
            vi = t_vol.fast_info
            hist = t_vol.history(period="90d")
            if len(hist) >= 30:
                avg_vol = hist['Volume'].iloc[:-1].mean()
                curr_vol = vi.get('base_volume', hist['Volume'].iloc[-1])
                if avg_vol > 0:
                    vq_pace = round(curr_vol / (avg_vol * pace_factor), 2)

        # 3. Range & Ampel
        h7 = t_price.history(period="7d")
        ...
        if vq_pace is not None:
            if vq_pace < 0.4: ampel = "yellow"
            elif vq_pace > 2.5: ampel = "green" if change >= 0 else "red"
            
        return {'name': name, 'symbol': symbol, 'ampel': ampel, 'price_val': curr, 'price': format_de(curr, 2 if "USD" not in symbol and "EUR" not in symbol else (0 if "BTC" in symbol else 4)), 'change_val': change, 'change': format_de(change, 2), 'vq': vq_pace, 'range_pos': round(range_pos, 0), 'is_pos': change >= 0, 'url': f"https://finance.yahoo.com/quote/{symbol}"}
    except: return None
...
def index():
    ...
    function copyData() {{
        ...
        navigator.clipboard.writeText(text.replace(/\\\\n/g, '\\n')).then(() => {{
            const btn = document.querySelector('.btn'); btn.innerText = "WERTE KOPIERT! ✅"; btn.style.background = "#4caf50";
            setTimeout(() => {{ btn.innerText = "SHORTCUT 2 ANALYSE AKTIV 🚀"; btn.style.background = "linear-gradient(45deg, #f1c40f, #f39c12)"; }}, 2000);
        }});
    }}
    </script></head><body>
    <div class="header"><span>VIX: <b>{format_de(vix_v)}</b></span><span>G/S: <b style="color:{gs_c};">{format_de(gs_val)}</b></span></div>
